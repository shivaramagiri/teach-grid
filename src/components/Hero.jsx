...
import WorkshopModal from "./WorkShop";
...
export default function Hero(){
  ...
  return (
    <>
     {/* Modal */}
      <WorkshopModal open={openWorkshop} onClose={() => setOpenWorkshop(false)} />
    <section className="max-w-6xl mx-auto px-6 py-16 flex flex-col md:flex-row items-center gap-12">
      <div className="flex-1">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="mt-4 text-lg text-slate-700">High-impact live classes, industry-grade projects, and personalized mentorship covering every skill you need to become a job-ready developer.</motion.p>

        <div className="mt-8 flex gap-4">
          <button className="px-6 py-3 rounded-xl bg-gradient-to-r from-pink-500 to-orange-400 text-white shadow-lg hover:scale-105 transition"  onClick={() => setOpenWorkshop(true)}>Join Free Workshop</button>
          <a href="#enroll" className="px-6 py-3 rounded-xl border-2 border-pink-200">View Curriculum</a>
        </div>
